...
def respond_send_email(respond_id):
    respond = Page.objects.get(id=respond_id)
    send_mail(
...
def respond_accept_send_email(response_id):
    respond = Page.objects.get(id=response_id)
    print(respond.post.author.email)
    send_mail(
        subject=f'MMORPG Billboard: Ваш отклик принят!',
        message=f'Здравствуйте, {respond.author}, Автор объявления {respond.post.title} принял Ваш отклик!\n'
    ...
    if list_week_posts:
        for user in User.objects.filter():
            ...
            send_mail(
                ...
                message=f'Здравствуйте, {user.username}!\nНовые объявление за последние 7 дней, {list_posts}',
                from_email='news@yandex.ru',
