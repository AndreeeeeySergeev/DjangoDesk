...
class PostItem(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if Page.objects.filter(author_id=self.request.user.id).filter(post_id=self.kwargs.get('pk')):
...
class EditPost(PermissionRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.request.user.username == 'admin' or self.request.user.username == author:
            ...
            return HttpResponse("Редактирование доступно только автору")
...
class Responses(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if title:
            ...
            context['filter_responses'] = list(Page.objects.filter(post_id=post_id).order_by('-dateCreation'))
            context['response_post_id'] = post_id.id
        else:
            context['filter_responses'] = list(Page.objects.filter(post_id__author_id=self.request.user).order_by(
                '-dateCreation'))
        context['myresponses'] = list(Page.objects.filter(author_id=self.request.user).order_by('-dateCreation'))
        return context
...
def response_accept(request, **kwargs):
    if request.user.is_authenticated:
        response = Page.objects.get(id=kwargs.get('pk'))
